...
def receive_commands(org_id: str) -> list[tuple[str, str, Optional[str]]]:
    """Poll every provider for inbound commands, resolving each sender to
    a `user_id` via `core.user_channels`. Returns `[(user_id, text,
    quoted_id), ...]` in arrival order across all providers; an
    unrecognized sender (no matching `user_channels` row) is silently
    dropped -- not a command, matching JA's own "not the owner -> not a
    command" rule generalized to "not a known user."
    ...
    for provider in PROVIDERS:
        ...
        for raw_sender, text, quoted_id in raw_commands:
            resolved_user_id = user_channels.resolve_user(org_id, provider, raw_sender)
            if resolved_user_id is None:
                continue
            commands.append((resolved_user_id, text, quoted_id))
    return commands
