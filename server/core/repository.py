...
| Operation | Applied |
|---|---|
| `list` / `get` / `count` | `require('read')` → read predicate in WHERE → `mask_record()` |
| `create` | `require('create')` → `reject_masked_writes()` → owner forced |
| `update` | `require('edit')` → `reject_masked_writes()` → **edit** predicate in the UPDATE |
...
def get_record(principal: Principal, entity: str, record_id: str) -> dict[str, Any]:
    ...
    return permissions.mask_record(principal, entity, dict(row))
...
def create(
    ...
    return permissions.mask_record(principal, entity, row)
...
def update(
    ...
    *,
    ...
    with pool.transaction(org_id=principal.org_id, user_id=principal.user_id) as cur:
        ...
        params: list[Any] = [columns[n] for n in columns]
        if custom or clear:
    ...
    _publish(event, event_id)
    return permissions.mask_record(principal, entity, after)
